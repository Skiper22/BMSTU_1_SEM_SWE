with open('in.txt', 'r') as inFile, open('out.txt', 'w') as outFile:
    ...
    # Сортировка методом пузырька с флагом относительно полученного
    # списка sumList
    for i in range(lenMatrixColumn - 1):
        flag = False
        for j in range(lenMatrixColumn - i - 1):
            if sumList[j] > sumList[j + 1]:
                sumList[j], sumList[j + 1] =\
                sumList[j + 1], sumList[j]
                count = 0
                # При сортировке списка sumList также сортируются столбцы
                # всей матрицы
                while count != lenMatrix:
                    matrix[count][j], matrix[count][j + 1] =\
                    matrix[count][j + 1], matrix[count][j]
                    count += 1
                flag = True
        if not flag:
            break
